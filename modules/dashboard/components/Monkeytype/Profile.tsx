...
const Profile = ({ data }: ProfileProps) => {
  ...
  // 🛡️ PELINDUNG TANGGAL: Fallback ke hari ini kalau addedAt kosong
  const date = new Date(data?.addedAt || Date.now());
  const endDate = new Date();
  const durationDays = differenceInDays(endDate, date);

  // 🛡️ PELINDUNG ANGKA WAKTU: Fallback ke 0
      ...
      <div className="flex flex-col gap-y-2">
        <div className="flex gap-x-4">
          <div className="flex items-center">
            {/* 💡 Catatan: Foto ini masih nembak ke satria.jpg, nanti jangan lupa diganti ya! */}
            <Image
          ...
          <div className="flex flex-col">
            ...
            <Tooltip
              title={`${t("current_streak")}: ${data?.maxStreak || 0} ${t("unit_days")}`}
            >
              <span className="text-xs text-neutral-600 dark:text-neutral-400">
                {t("current_streak")}: {data?.streak || 0} {t("unit_days")}
              </span>
            </Tooltip>
          </div>
        </div>
